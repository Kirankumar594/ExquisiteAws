...
export const createMusicSection = async (req, res) => {
  try {
    const { title, musicId, type } = req.body;
    const image = req.files['image']?.[0] ?  await uploadFile2(req.files['image']?.[0],"musicsection"):""
    const media = req.files['media']?.[0] ? await uploadFile2(req.files['media']?.[0],"musicsection"):""

    if (!title || !musicId || !type || !image || !media) {
      return res.status(400).json({ message: 'All fields are required' });
    }
    if (!['audio', 'video'].includes(type)) {
      return res.status(400).json({ message: 'Invalid type, must be audio or video' });
    }

    const newSection = new MusicSection({ title, musicId, type, image, media });
